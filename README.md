...
The frontend expects the API to ve available at 'http://localhost:400' by default.
...
- `src/environments/environment.prod.ts` (productive build)
...
If you want to run everything fully in the browser (no bakcend), you can enable the built-in fake backend interceptor.
...
```
   ts
    providers: [
        ...
        { provide: HTTP_INTERCEPTOR, useClass: JwtInterceptor, multi: true },
...
- Accounts are stored  in your browser `localStorage`, not in the database.
...
- The first registered account becomes `Admin`, and all other accounts bevome `User`.
...
3. If you are using the fake backend, a "verfication email" will appear as an alert with a link
